...
var Vector = function(x, y) {   //私有属性  横向速度x ,纵向速度y
	  
	this.x = x || 0;  
	this.y = y || 0;
};
...
Vector.prototype.copy = function() {   //返回一个同等速度属性的Vector实例
	  
...
//构造函数 Drop
var Drop = function() {   //随机设置drop的初始坐标 
	   //首先随机选择下落对象是从从哪一边
	  
	...
	  
	this.prev = this.pos;   //将角度乘以 0.017453293 （2PI/360）即可转换为弧度。
	  
	var eachAnger =  0.017453293;    //获得风向的角度
	  
	wind_anger = OPTS.wind_direction * eachAnger;   //获得横向加速度 
	  
	speed_x =  this.speed * Math.cos(wind_anger);   //获得纵向加速度
	  
...
Drop.prototype.update = function() {
	...
	   
	this.prev = this.pos.copy();   //如果是有重力的情况，则纵向速度进行增加
	   
	if(OPTS.hasGravity) {     
		this.vel.y += gravity;   
	}  //
...
<span style = "color: rgb(248, 248, 242); font-family: 'Source Code Pro', Consolas, Monaco, Menlo, Consolas, monospace; line-height: 22px; white-space: pre-wrap; background-color: rgb(39, 40, 34);" > < /span>Drop.prototype.draw = function() { 
ctx.beginPath(); 
...
}; < span style = "color: rgb(248, 248, 242); font-family: 'Source Code Pro', Consolas, Monaco, Menlo, Consolas, monospace; line-height: 22px; white-space: pre-wrap; background-color: rgb(39, 40, 34);" > < /span>
...
function update() {  
	...
	while(i--) {    
		var drop = drops[i];    
		drop.update();     //如果drop实例下降到底部，则需要在drops数组中清楚该实例对象
		    
		if(drop.pos.y >= canvas.height) {       //如果需要回弹，则在bouncess数组中加入bounce实例
			      
			if(OPTS.hasBounce) {        
...
function init(opts) {  
	...
	ctx = canvas.getContext("2d");   ////兼容高清屏幕，canvas画布像素也要相应改变
	  
	DPR = window.devicePixelRatio;   //canvas画板像素大小， 需兼容高清屏幕，故画板canvas长宽应该乘于DPR
	  
	canvasWidth = canvas.clientWidth * DPR;  
	canvasHeight = canvas.clientHeight * DPR;   //设置画板宽高
	  
	canvas.width = canvasWidth;  
	canvas.height = canvasHeight;  
	drop_chance = 0.4;   //设置样式
	  
	setStyle();
}
